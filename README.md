...
- Without a native `ResizeObserver` (very old browsers, or a bare non-browser environment), the instance takes one synchronous measurement on creation and does not track further resizes automatically. `update()` re-measures only when the patch carries an explicit `size: 'fill'` or changes `shape` (an unrelated update, e.g. `update({ fg })`, does not) — call `update({ size: 'fill' })` again to force one.
